...
function randomkey(key) {
    ...
    for(var i = 0;i < key.length/2 ;i+=8 ) {
        ...
        result += tempnum.length == 8 ? tempnum : repeat("0",8 - tempnum.length) + tempnum;
    }
    return result;
}
...
function bigxor(a, b) {
    ...
    abin = abin.length == loggest ? abin : repeat("0",loggest - abin.length) + abin;
    bbin = bbin.length == loggest ? bbin : repeat("0",loggest - bbin.length) + bbin;
    var result = "";
...
function test(){
    ...
    sm4 += "message: 0123456789abcdeffedcba9876543210";
    key = "0123456789abcdeffedcba9876543210";
    sm4 += "<br>key: "+key;
    var start = new Date().getTime();
    ciphertext = sm4_encode_cbc("0123456789abcdeffedcba9876543210",key);
    var second = new Date().getTime();
    var result = "";
    for(var i=0;i<ciphertext.length;i++){
        hextext = ciphertext[i].toString(16);
        result += hextext.length == 16 ? hextext :repeat("0",8-hextext.length)+hextext;
    }
...
function encrypt_sms4(plaintext){
    var ciphertext = sm4_encode_cbc(plaintext, key);
    var result = "";
    for(var i=0;i<ciphertext.length;i++){
        hextext = ciphertext[i].toString(16);
        result += hextext.length == 16 ? hextext :repeat("0",8-hextext.length)+hextext;
...
function decrypt_sms4(ciphertext){
    var text = new Array();
    for(var i=0;i<ciphertext.length;i+=8){
        text.push(parseInt(ciphertext.slice(i,i+8),16));
    }
    plaintext = sm4_decode_cbc(text,key);
    return plaintext;
}
