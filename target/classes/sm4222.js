...
function leftshift(a, n) {
	...
	bin = bin.length == size ? bin :repeat("0",size - bin.length) + bin;
...
function PUT_ULONG_BE(b) {
	b = sm4Sbox(b);
	var resullt = bigxor(b, bigxor(leftshift(b, 13), leftshift(b, 23)));
	return resullt;
}
...
function encrypt_sm4 (messsage, key,  method) {
	var MK = key;
...
function decrypt_sm4 (ciphertext, key, method) {
    var MK = key;
    var X = ciphertext;
...
function jiami(){
	...
	var message = new Array(0x01234561,0x89abcdef,0xfedcba98,0x76543210);
